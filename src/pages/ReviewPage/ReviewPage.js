    ...
    render() {
            ...
            for (var i = 0; i < this.props.items.length; i++) {
                items.push( <
                    ...
                    <
                    span class = "page-info-item-title" > {
                        this.props.items[i]['title']
                    } < /span> <
                    span class = "page-info-item-content" > {
                        this.props.items[i]['content']
                    } < /span> <
                    /div>
                )
            }
            ...
            for (var i = 0; i < this.props.tableitems.length; i++) {
                ...
                table.push( < tr > < td class = {
                        tableHeaderClass
                    }
                    colSpan = {
                        4
                    } > {
                        this.props.tableitems[i]['title']
                    } < /td></tr > );
                for (var j = 0; j < this.props.tableitems[i]['items'].length; j += 2) {
                    var row = [];
                    row.push( < td class = "item-icon item-l" > < a href = {
                                this.props.tableitems[i]['items'][j]['link']
                            } > < FontAwesomeIcon icon = {
                                this.fileIcon(this.props.tableitems[i]['items'][j]['type'])
                            }
                            /></a > < /td>);
                            row.push( < td class = 'item-content item-r' > {
                                    this.props.tableitems[i]['items'][j]['title']
                                } < /td>);
                                if (j + 1 < this.props.tableitems[i]['items'].length) {
                                    row.push( < td class = "item-icon" > < a href = {
                                            this.props.tableitems[i]['items'][j + 1]['link']
                                        } > < FontAwesomeIcon icon = {
                                            this.fileIcon(this.props.tableitems[i]['items'][j + 1]['type'])
                                        }
                                        /></a > < /td>);
                                        row.push( < td class = 'item-content item-r' > {
                                                this.props.tableitems[i]['items'][j + 1]['title']
                                            } < /td>);
                                        }
                                        else {
                                            ...
                                            return ( <
                                                ...
                                                <
                                                CVStatCard header = "Company Sector"
                                                primary = {
                                                    this.props['sector']
                                                }
                                                /> <
                                                CVStatCard header = "Amount Requested"
                                                primary = {
                                                    this.props['amount']
                                                }
                                                subtitle = "USD" / >
                                                <
                                                CVStatCard header = "Geographic Location"
                                                primary = {
                                                    this.props['location']
                                                }
                                                ...
                                                this.data = {
                                                    ...
                                                    items: [{
                                                        ...
                                                        },
                                                        {
                                                            title: "Loan Purpose:",
                                                            content: ( < div > < span > {
                                                                    "• T into new product development. "
                                                                } < /span><span>{"• 	Training"}</span > < /div> )
                                                            },
                                                            {
                                                                title: "Loan Purpose:",
                                                                content: ( < div > < span > {
                                                    ...
                                                    pageLead() {
                                                        return ( <
                                                            OrganizationLead backgroundImage = {
                                                                this.data.image
                                                            }
                                                            title = {
                                                                this.data.title
